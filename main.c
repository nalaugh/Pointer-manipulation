...
int main(int argc, char *argv[]) {
    ...
    int*fine = malloc(sizeof(int)* numLine);
    int*origingal = malloc(sizeof(int)* numLine);
    ...
    while (lines <numLine){
        ...
        *(line+totalnumber)= malloc( sizeof(int)*numbers);
        int  set[numbers];
        *(fine+lines) = numbers;
    ...
    stringSort(fine, numLine,numsstrcmp);//forting the line length
    printf("This shows the length before and after the sort\n");
    printArray(fine, numLine);//printing the sorted
    printArray(origingal, numLine);//printing the origingal order
    int k=0;//marker
    for(int j =0; j<numLine;j++)  {
        k = 0;
        while(*(fine+j)!=*(origingal+k)){
            k++;
        }
    ...
    for(int i = 0; i<numLine;i++){
        printArray((line+((origingal[i]))), *(fine+i));//Print the final array
    }
    //freeing the memory
free(line);
    free(fine);
